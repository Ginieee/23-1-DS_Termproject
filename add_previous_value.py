...
def add_previous_price_feature(df_list):
    ...
    for df in df_list:
        ...
        for idx in df.index:
                if type(df.loc[idx, '일시']) == "datetime.datetime":
                   df.loc[idx, '일시'] =  datetime.strptime(df.loc[idx, '일시'], date_format)
...
# Save Dataset
def add_previous_save(data_list, item_list, file_path, df_name):
    for df, item in zip(data_list, item_list):
        saveDataset(df, file_path, item+df_name)
    return data_list